...
btnStartGame.addEventListener("click", function () {
    if (isNaN(inputMoney.value) || inputMoney.value <= 0 || inputMoney.value < 100) {
        alert("Please enter a valid bet amount (min.100 bet)");
        return
    }

    if (btnStartGame.textContent === "TRY AGAIN?") {
        if (yourMoney.textContent <= 0) {
            alert("Minggir lu miskin. Tobattt!");
            return btnStartGame.textContent = "PLAY AGAIN?";
        }

        if (yourMoney.textContent < parseInt(inputMoney.value)) {
            alert("Gak punya duit tah? Depo aja skuy");
    ...
    if (cardsLeft.textContent < 4) {
        alert("Kartu Habis Silahkan Klik Reset Card")
        return;
    }
        ...
        setTimeout(function () {
            ...
            if (aiSums < 18) {
                ...
                if (yourSums > 21) {
                    ...
                    if (yourSums == 21) {
                        message = "You win!";
                        yourMoney.textContent = parseInt(yourMoney.textContent) + (inputMoney.value * 4 / 2);
                    } else if (yourSums < 21) {
                        message = "You win!";
...
function getValueOfCard(card) {
    ...
    if (isNaN(value)) {
        if (value === "A") {
            for (let i = 0; i <= 21; i++) {
                if (yourSums + 11 <= 21 || aiSums + 11 <= 21) {
                    return 11; 
                } else {
                    return 1; 
                }
