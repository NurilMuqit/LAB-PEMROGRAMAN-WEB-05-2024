...
<body class="min-h-screen bg-[#0f1523]">
    <div class="container mx-auto px-4 ">

        <?php if ($role == 'admin'): ?>
            <h1 class="text-white text-center md:text-left text-[2rem] lg:text-[5rem] font-bold">Selamat datang, <span class="bg-gradient-to-tl from-slate-800 via-violet-500 to-zinc-400 bg-clip-text text-transparent"><?= $firstName; ?></span></h1>
            <div class="mb-9">
                <form method="POST" class="mb-4 mt-4 flex flex-col md:flex-row items-center gap-3">
                    <input class="w-full md:w-2/5 bg-transparent shadow appearance-none border-b-2 border-white py-2 px-3 text-white text-lg focus:outline-none focus:border-b-2" type="text" name="keyword" placeholder="Cari mahasiswa...">
                    <button class="w-full md:w-24 bg-green-900 bg-opacity-75 border border-green-700 hover:bg-green-700 hover:border-green-500 text-white font-bold py-2 px-4 rounded" type="submit" name="cari">Cari</button>
                    <button class="w-full md:w-24 bg-gray-900 bg-opacity-75 border border-gray-700 hover:bg-gray-700 hover:border-gray-500 text-white font-bold py-2 px-4 rounded" type="submit" name="reset">Reset</button>
                </form>
            </div>
            ...
            <?php if (isset($editData)): ?>
                <form name="mahasiswaForm" onsubmit="return validateForm()" method="POST" class="bg-transparent text-white mb-4 mt-4 shadow-md border-2 border-yellow-900 rounded px-4 py-4 max-w-lg">
                    ...
                    <button class="bg-yellow-900 bg-opacity-75 border border-yellow-700 hover:bg-yellow-700 hover:border-yellow-500 text-white font-bold py-2 px-4 rounded" type="submit" name="update">Update</button>
                    <a href="index.php" class="bg-gray-900 bg-opacity-75 border border-gray-700 hover:bg-gray-700 hover:border-gray-500 text-white font-bold py-2 px-4 rounded">Batal</a>
                </form>
            ...
            <?php elseif ($showTambahForm): ?>
                <form name="mahasiswaForm" onsubmit="return validateForm()" method="POST" class="bg-transparent text-white mb-4 mt-4 shadow-md border-2 border-green-900 rounded px-4 py-4 max-w-lg">
                    ...
                    <?php if (isset($error)) { echo "<div class='mb-4 p-2 bg-red-100 border border-red-400 text-red-700 rounded'>$error</div>"; } ?>
                    <button class="bg-blue-900 bg-opacity-75 border border-blue-700 hover:bg-blue-700 hover:border-blue-500 text-white font-bold py-2 px-4 rounded" type="submit" name="tambah">Tambah</button>
                    <a href="index.php" class="bg-gray-900 bg-opacity-75 border border-gray-700 hover:bg-gray-700 hover:border-gray-500 text-white font-bold py-2 px-4 rounded">Batal</a>
                </form>
            <?php else: ?>
                <div class="mb-7">
                    <a href="index.php?tambah" class="bg-green-900 bg-opacity-75 border border-green-700 hover:bg-green-700 hover:border-green-500 text-white font-bold py-2 px-4 rounded mt-8">Tambah Mahasiswa</a>
                </div>
            <?php endif; ?>
        <?php endif; ?>

        <?php if ($role == 'admin'): ?>
            <h3 class="text-xl mb-4 text-center mt-8 text-gray-600">Data Mahasiswa:</h3>
                        ...
                        while ($row = mysqli_fetch_assoc($mahasiswaResult)): ?>
                            <tr class="odd:bg-slate-900 even:bg-transparent">
                                ...
                                <td class="border-b-[1px] border-dashed px-4 py-4 flex flex-col md:flex-row gap-2">
                                    <a href="index.php?edit=<?= $row['NIM']; ?>" class="bg-yellow-900 bg-opacity-75 border border-yellow-700 hover:bg-yellow-700 hover:border-yellow-500 text-white font-bold py-2 px-4 rounded">Edit</a>
                                    <a href="index.php?hapus=<?= $row['NIM']; ?>" onclick="return confirm('Apakah Anda yakin?')" class="bg-red-900 bg-opacity-75 border border-red-700 hover:bg-red-700 hover:border-red-500 text-white font-bold py-2 px-4 rounded">Hapus</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
            ...
            <div class="flex justify-center mt-4">
                <nav>
                    <ul class="flex space-x-2">
                        <?php if ($page > 1): ?>
                            <li>
                                <a href="index.php?page=<?= $page - 1; ?>" class="bg-gray-900 bg-opacity-75 border border-gray-700 hover:bg-gray-700 hover:border-gray-500 text-white font-bold py-2 px-4 rounded">Prev</a>
                            </li>
                        <?php endif; ?>
                        <?php for ($i = 1; $i <= $totalMahasiswaPages; $i++): ?>
                            <li>
                                <a href="index.php?page=<?= $i; ?>" class="bg-gray-900 bg-opacity-75 border border-gray-700 hover:bg-gray-700 hover:border-gray-500 text-white font-bold py-2 px-4 rounded <?= $page == $i ? 'bg-blue-500 text-white' : ''; ?>">
                                    <?= $i; ?>
                                </a>
                            </li>
                        <?php endfor; ?>
                        <?php if ($page < $totalMahasiswaPages): ?>
                            <li>
                                <a href="index.php?page=<?= $page + 1; ?>" class="bg-gray-900 bg-opacity-75 border border-gray-700 hover:bg-gray-700 hover:border-gray-500 text-white font-bold py-2 px-4 rounded">Next</a>
                            </li>
                        <?php endif; ?>
                    </ul>
                </nav>
        ...
        <div class="mt-7 flex justify-center align-items">
            <a href="logout.php" class="w-36 text-center bg-red-900 bg-opacity-75 border border-red-700 hover:bg-red-700 hover:border-red-500 text-white font-bold py-2 px-4 rounded">Logout</a>
        </div>
